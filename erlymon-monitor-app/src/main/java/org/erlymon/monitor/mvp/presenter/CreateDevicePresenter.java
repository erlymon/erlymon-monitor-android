...
public class CreateDevicePresenter extends BasePresenter<CreateDeviceView> {
    ...
    public void save(Device device) {
        getViewState().startCreateDevice();

        // save session id
        Subscription subscription = mMainService.createDevice(device)
                .subscribe(result -> {
                    getViewState().finishCreateDevice();
                    getViewState().successCreateDevice();
                }, exception -> {
                    getViewState().finishCreateDevice();
                    getViewState().failedCreateDevice(exception.getMessage());
                });
