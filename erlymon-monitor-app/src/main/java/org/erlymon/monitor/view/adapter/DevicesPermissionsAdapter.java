...
public class DevicesPermissionsAdapter extends ArrayAdapter<Device> {
    ...
    public DevicesPermissionsAdapter(Context context, Pair<Device[], Device[]> data, PermissionListener permissionListener) {
        super(context, R.layout.list_device_permission, Arrays.asList(data.first));
        for (Device device: data.second) {
